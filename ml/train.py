...
Built incrementally, task by task (see TASKS.md Phase 5 & 6 /
INTERVIEW_PREP.md for the reasoning behind each decision). This file is run
directly (`python -m ml.train`), not imported as a library — it's a script,
matching the PDF's suggested `ml/train.py` structure.

T093/T094 (this pass): load the 3 modeling tables from Postgres and define
the target. Loaded from the DB, not the raw CSVs again — Phase 3 already
reconciled the loaded data against `dataset_statistics.csv` exactly, so the
DB is the trustworthy source now, and reusing `app/db/session.py` keeps one
connection-config path for the whole project instead of a second one here.

Deliberately NOT loaded: `ratings`/`customers`. `ratings` is excluded from
this pipeline entirely (see SCOPE.md's 2026-09-13 change-log entry) — every
returned order has `delivery_status='Cancelled'`, the same state that
blocks a rating from ever existing (confirmed: 0 of 9,462 returned orders
have one), so any rating-derived feature would leak the target.
`customers` isn't needed for T093/T094 either — customer-history features
(T100) are recomputed from `orders` itself, not read off the `customers`
table.
"""
import json
...
# ----------------------------------------------------------------------------
# T093 — load orders, order_items, products into pandas
# ----------------------------------------------------------------------------
...
# Sanity check against the row counts already reconciled in Phase 3 — if
...
print("row counts match the Phase 3 reconciled totals - OK")

# ----------------------------------------------------------------------------
# T094 — define the target: order_status == 'Returned'
# ----------------------------------------------------------------------------
# `return_status IS NOT NULL` agrees with this exactly (see SCOPE.md's
# "Known facts") — using `order_status` since it's the one NOT NULL column
...
# ----------------------------------------------------------------------------
# T095 — time-based split (train 2021-24, test 2025)
# ----------------------------------------------------------------------------
...
# prints/asserts above only. T100 below sorts `orders` and resets its index,
# which moves rows to new positions/labels - these masks get recomputed
# from `order_year` again right after that sort, and it's the recomputed
# pair (not these) that T102 actually uses. See the bug note there and in
# INTERVIEW_PREP.md (2026-09-13, "T103: bug found").

# ----------------------------------------------------------------------------
# T096 — feature: discount ratio — EXCLUDED (target leak, found at T114)
# ----------------------------------------------------------------------------
# Originally built as discount_amount / gross_sales and used by every model
# through T109. Removed after the T114 smoke test showed the model's output
# was almost entirely a step function on "discount_ratio == 0": in this
...
# delivery_days/ratings (T098), just hidden inside a ratio. Full evidence in
# INTERVIEW_PREP.md (2026-09-13, "CRITICAL: discount_ratio is a severe
# target leak").
#
...
assert orders.loc[_discounted, "target"].sum() == 0, \
    "discount_amount > 0 now co-occurs with returns - the T096 leak may be gone, reconsider the feature"
assert (orders.loc[orders["target"] == 1, "discount_amount"] == 0).all(), \
    "a returned order now has a non-zero discount - the T096 leak may be gone, reconsider the feature"
...
# caution given to the T100 fields below even though it isn't excluded:
...
# uses `order_items`/`products` at all, even though T093 loads both. An
...
# Actual one-hot/encoder fitting happens in T102, on the training split only
# - encoding is prepared here, not fit here.

# ----------------------------------------------------------------------------
# T100 — customer-history features, recomputed point-in-time (NOT the
# dataset's own is_repeat_customer/customer_order_count/
# customer_lifetime_value columns - see SCOPE.md's 2026-09-13 change log)
# ----------------------------------------------------------------------------
...
# BUGFIX (found while sanity-checking T103, logged in INTERVIEW_PREP.md
# 2026-09-13): the sort above reorders rows and resets the index to a fresh
# 0..N-1 - the `train_mask`/`test_mask` computed back in T095 are indexed
# 0..N-1 in the *pre-sort* row order, so `.loc[train_mask, ...]` in T102
# would align by label onto the wrong rows post-sort (same counts, wrong
# content - it passed every shape assertion while silently leaking 2025
# orders into "train" and vice versa). Recomputing from `order_year` here,
# after the sort, is what T102 actually uses - `order_year`'s per-row
# values never changed, only the row positions did, so recomputing (not
# reordering the mask) is the correct fix.
train_mask = orders["order_year"] <= 2024
test_mask = orders["order_year"] == 2025
assert train_mask.sum() == 110_518, f"expected 110,518 train orders, got {train_mask.sum()}"
assert test_mask.sum() == 27_598, f"expected 27,598 test orders, got {test_mask.sum()}"
...
# ----------------------------------------------------------------------------
# T101 — missing-value handling strategy
# ----------------------------------------------------------------------------
...
# missingness in `orders` (T098's exclusion, and ratings was never joined
# in T093) - are simply not part of the feature set. What's left is audited
...
# ----------------------------------------------------------------------------
# Leakage audit (added after the T096 discount_ratio leak) — every feature,
...
# ----------------------------------------------------------------------------
# T102 — preprocessing pipeline: two separate paths, one per model
# ----------------------------------------------------------------------------
...
def build_preprocessor_linear() -> ColumnTransformer:
    ...
    up to 3.67 (T097), which would distort a mean/variance-based scaler.
...
print("\nT102 preprocessing verified on both paths - OK")
...
# balanced weighting on a ~7% base rate, and is why T105 treats the
# 0.5-threshold metrics as secondary to the threshold-free ones.

print("\nT103 baseline Logistic Regression trained and sanity-checked - OK")

# ----------------------------------------------------------------------------
# T104 — train challengers: HistGradientBoostingClassifier, Random Forest,
# XGBoost
# ----------------------------------------------------------------------------
# Broadened beyond SCOPE.md's original "LR vs. HGB" after the T096 leak was
# fixed and both models turned out only marginally above random - before
# finalizing on a near-random result, the user asked to confirm it with two
# more model families rather than stop at one comparison (approved scope
# expansion, logged in SCOPE.md's change log and INTERVIEW_PREP.md
# 2026-09-13). All four models share the same features, split, and
...
#
# --- HistGradientBoostingClassifier (kept from the original T104) ---
# `categorical_features="from_dtype"` auto-detects T102's `to_tree_format()`
...
# capacity finding noise, not signal," consistent with T106's overall
# conclusion.
...
# overfitting for). Full variant comparison table in INTERVIEW_PREP.md
# (2026-09-13, "third/fourth model families").
...
# Per-model train probability/prediction arrays, needed by T105's
# evaluate() calls below (and, for HGB/XGBoost, kept under their original
# T104-era names since other code/comments still refer to them that way).
...
print("\nT104 challengers (HGB, RandomForest, XGBoost) trained and sanity-checked - OK")
...
# T106's model selection, since train-set metrics only measure fit, not
...
# is exactly why PR-AUC/ROC-AUC (threshold-free, ranking-based) are
# SCOPE.md's primary metrics, not these. PR-AUC is computed via
...
# a sanity floor, not a claim about which model is better (that's T106).
...
print("\nT105 evaluation complete for all four models - all metrics in valid range - OK")

# ----------------------------------------------------------------------------
# T106 — compare & select final model, write rationale
# ----------------------------------------------------------------------------
# Final choice: XGBoost (regularized — T104), across four model families
...
#     ROC-AUC ranges 0.51-0.54 against 0.5. Before the T096 leak was found,
...
#     way it was for HGB in T104, selecting on the gap+test-PR-AUC
...
#     way none of the three tree-based models are. Not a blocker: T111's
...
#     currently used - a candidate follow-up, deliberately not bolted on
#     mid-rework without sign-off (SCOPE.md rule: no silent scope changes).
...
print(f"\n[T106] FINAL MODEL SELECTED: {FINAL_MODEL_NAME} {FINAL_MODEL_HYPERPARAMS}")
print("[T106] test PR-AUC by model:", {k: round(v["pr_auc"], 4) for k, v in ALL_MODELS_TEST_METRICS.items()})
print("[T106] test ROC-AUC by model:", {k: round(v["roc_auc"], 4) for k, v in ALL_MODELS_TEST_METRICS.items()})
print("[T106] train/test PR-AUC gap by model:", {
...
print("\nT106 final model selected (of 4 families compared) and rationale recorded - OK")
...
# T110's prediction API needs to load. Its metrics are still preserved in
...
# reads category *codes* off the pandas dtype at fit time. If T110's
...
# exact trained category order here means T110 can pin it explicitly
...
# Reference stats for T111's rule-based "contributing factors" heuristic in
...
metadata = {
    ...
    "risk_reference_stats": {
        "note": "Computed from the TRAIN split only (T111's contributing-factors heuristic must "
    ...
    "overfitting_check": {
        "note": "Regularization search run for both tree ensembles: HGB's l2_regularization=1.0/"
                ...
                "the reason it was rejected, not tuned further. Full detail in INTERVIEW_PREP.md "
                "2026-09-13.",
    ...
    "excluded_features": {
        "discount_ratio": "TARGET LEAK - discount_amount is 0 for 100% of Returned and Cancelled orders "
                          "and >0 for ~100% of Completed orders (data-generation artifact). Found at T114, "
        ...
        "delivery_days / estimated_delivery_days / delivery_status": "NULL/'Cancelled' for 100% of "
                          "returned orders - leaks the outcome (T098).",
        ...
        "customer_lifetime_value / customer_order_count / is_repeat_customer (raw)": "lifetime "
                          "aggregates, not point-in-time snapshots (T100); recomputed as-of-order instead.",
...
# artifact at all (T110 would fail much later, harder to diagnose).
...
print("\nT107 model artifact + metadata saved and verified - OK")
...
# matching what T105/T106 actually compared) - a single-model plot would
...
print("\nT108 eval plots saved and verified on disk - OK")

# ----------------------------------------------------------------------------
# T109 (delighter) — generate ml/MODEL_CARD.md from training metadata
# ----------------------------------------------------------------------------
...
print("\nT109 MODEL_CARD.md generated from metadata and verified on disk - OK")
